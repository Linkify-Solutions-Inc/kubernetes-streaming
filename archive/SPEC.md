...
2. **Phase 2: Kubernetes (dev)** — ~~install kubeadm on the Ubuntu box~~ **done 2026-07-28** (cluster + Calico + MetalLB + ingress-nginx, see "Phase 2 cluster" above). Then, roughly in dependency order:
   ...
   5. **`transcode-worker` becomes the container image run by two `KEDA ScaledJob`s** (live off a new `stream.start.requests` topic, VOD off existing `upload.events`) instead of a long-running Deployment — **done** (see `docs/aws/14-keda-scaledjobs.md`): `main()` is a claim-one-message-and-exit dispatcher (`TRANSCODE_MODE=live|vod`), the Postgres claim + heartbeat replaces the in-memory `active_streams` dict, a `sweeper.py` CronJob re-queues abandoned claims, and `ingest-webhook` gates `/hooks/auth` on `MAX_CONCURRENT_LIVE_STREAMS`. Resource `requests`/`limits` sized from this session's empirical benchmark (~2.3 cores / ~1GB RAM per live job).
