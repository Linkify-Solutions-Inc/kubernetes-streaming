...
3. **CI/CD** — not started; see the settled decision above for the full design. Can start in parallel with Phase 2, not blocked by it:
   1. **CI**: GitHub Actions workflow — `ruff` + `hadolint`, the 3 starting tests, `paths-filter`-driven matrix build, push to `ghcr.io`.
   2. **Phase 1 CD**: self-hosted runner on the box (systemd service, restricted user, `Nice=`/`CPUWeight=` constrained) running `docker compose pull/up`; switch the box's compose files from `build:` to `image: ghcr.io/...:sha`.
