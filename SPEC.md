...
- **Live transcode scaling (Phase 2)**: one Kubernetes Job/pod per live stream, not a shared pool of `transcode-worker` replicas — this is how real streaming platforms isolate concurrent transcodes, and it's a better fit than horizontally scaling today's multi-stream-per-process worker. A small Python dispatcher service consumes `stream.lifecycle`, and on each `started` event calls the Kubernetes API to create a Job scoped to that one `stream_id` (passed in as a pod env var alongside the RTMP path). The pod runs today's ffmpeg/upload logic, just scoped to a single stream, and consumes `stream.lifecycle` itself, filtering for its own `stream_id`'s `ended` event to SIGTERM ffmpeg and exit cleanly — the dispatcher only ever creates pods, it never has to track or tear one down. VOD uploads are explicitly out of scope for this pattern (each is already a short, one-shot transcode); they keep running through a small pool of the existing shared `transcode-worker`. This pod is a single **writer** only (ingest + transcode) — it has no relationship to viewer count. Viewers read finished HLS segments straight from MinIO/S3 (or a CDN later) over plain HTTP, a completely separate and independently-scaling path; one viewer or ten thousand is identical load on the transcode pod, since it never serves them directly.
- **Networking / load balancing (Phase 2)**: two different traffic shapes need two different mechanisms. HTTP services (`web`, `upload-api`, `ingest-webhook`) sit behind a single Ingress controller doing L7 routing (host/path rules, TLS termination) across each service's replicas, replacing today's separate host ports. RTMP (MediaMTX, port 1935) isn't HTTP, so it needs L4 TCP load balancing instead: a `LoadBalancer`/`NodePort` Service, not an Ingress. Per-stream transcode pods need no inbound load balancing at all since they only pull from MediaMTX and push to S3, never receiving inbound traffic themselves. The dispatcher runs as a normal Kafka consumer group, so if it's ever scaled to multiple replicas for its own HA, Kafka's own partition assignment already prevents two replicas from double-creating a Job for the same stream, no separate leader-election mechanism needed.
